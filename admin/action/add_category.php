<?php
    ...
    $color_code=$db->real_string($color_code);
    ...
    $res['dpl']=false;
    $res['edit']=false;
    $dpl=$db->duplicate("*","category","en_name = '$en_name' && id != '$edit_id'");
    if($dpl==false){
        if($edit_id==0){
            $tbl="category";
            $val=" NULL,'$date_post','$poster','$en_name','$kh_name','$color_code','$od','$status' ";
            ...
            $tbl="category";
            $con="id = $edit_id";
            $fld="uid = '$poster' ,en_name = '$en_name',kh_name='$kh_name',color_code='$color_code', od='$od' , status='$status' ";
